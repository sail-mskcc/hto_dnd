...
def test_dnd(mock_hto_data):
    ...
    def assert_results(adata):
        ...
        assert isinstance(adata, ad.AnnData), (
            "Demultiplexing result is not an AnnData object"
        )
        assert "hash_id" in adata.obs.columns, (
            "'hash_id' column not found in demultiplexing result"
        )
        assert "doublet_info" in adata.obs.columns, (
        ...
        assert "pseudocount" in adata.uns["dnd"]["normalise"]["params"], (
            "Pseudo count not found in normalisation result"
        )
        assert adata.uns["dnd"]["normalise"]["params"]["pseudocount"] == 20, (
            "Pseudo count is not 20"
        )
        ...
        assert all(
            classifications.index.isin(["negative", "doublet"] + list(hash_ids))
        ), "Invalid classifications found"
        ...
        assert overall_accuracy > 0.8, (
            f"Overall accuracy is only {overall_accuracy:.2f}"
        )
