...
def hto_and_gex():
    ...
    adata_hto.X = adata_hto.X.astype(x_hto.dtype)
    adata_gex.X = adata_gex.X.astype(x_gex.dtype)
...
def hto_and_raw():
    ...
    adata_hto.X = adata_hto.X.astype(x_hto.dtype)
    adata_hto_raw.X = adata_hto_raw.X.astype(x_hto.dtype)
...
def test_build_background_v2(hto_and_raw):
    ...
    adata_background = hto.tl.build_background(
        "v2", adata_hto=adata_hto, adata_hto_raw=adata_hto_raw, _run_assert=False
    )
    ...
    with pytest.raises(UserInputError):
        adata_background = hto.tl.build_background(
            "v2", adata_hto=adata_hto, adata_hto_raw=adata_hto, _run_assert=False
        )
